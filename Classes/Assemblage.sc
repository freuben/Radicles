	...
	*new {arg trackNum=1, busNum=0, chanNum=1, spaceType, memSize, action;
	...
	addTrack {arg type=\track, chanNum=1, spaceType, trackSynth;
		...
		if([\track, \bus, \master].includes(type), {
			...
			ndefCS.radpost.interpret;
			ndefCS = ("Ndef(" ++ trackInTag.cs ++ ").set('trim', 0);");
			ndefCS.radpost.interpret;
			server.sync;
	...
	bus {arg trackNum=1, busNum=1, mix=1, trackType=\track, action, setVal=true;
	...
	convRevBuf {arg filterTag, impulse=\ortf_s1r1, fftsize=2048, inVar,
		...
		string, filterBuffArr, wcond, dash;
		{
			...
			path = soundFilePath ++ dash++ "IR" ++ dash ++ impulse ++ ".wav";
			file = SoundFile.new;
			file.openRead(path);
	...
	getBusInArr {arg thisArr;
		var busLabelArr, sameTracks, result;
		thisArr.do{|item|
			if(item[1].notNil, {
				if(item[1].isArray, {
					item[1].do{|it|
...
/*		panKnobTextArr.size.postln;
		...
		fxSlotArr.do{|item, index|
			item.do{|it, ind|
				...
				fltWinFunc2 = {arg menu, thisListView, irItems, labelKey;
					...
					irItems = PathName(soundFilePath ++ dash ++ "IR" ++ dash).entries
	...
	saveVars {
		^[inputs, space, masterNdefs, masterSynth, trackNames, masterInput, busArr,
