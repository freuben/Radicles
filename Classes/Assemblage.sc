...
writeTracksPreset {arg presetName;
	var dataArr, filterKeyArr, mixDataArr;
	...
	if(presetName.notNil, {
		PresetFile.write(\tracks, presetName, [mixDataArr, dataArr]);
	});
}
...
modRawPreset {arg newFilterNdef, hasMod, action;
	var firstNdef;
...
prepLoadTrackPreset {arg trackType, trackNum, dataArr, cond;
...
loadTrackPreset {arg trackType, trackNum, presetName;
	var dataArr, cond, modSettings, fxSettings, arrSettings;
	{
		...
		arrSettings = this.prepLoadTrackPreset(trackType, trackNum, dataArr, cond);
...
mixControlKeyValues {
	var dataArr;
	dataArr = mixTrackNames.collect({|item|
		var in, space;
		in = ("in" ++ item.asString.capitalise).asSymbol;
		space =("space" ++ item.asString.capitalise).asSymbol;
		[ this.rawFxPreset(in, false),
			this.rawFxPreset(item, false),
			this.rawFxPreset(space, false)
		];
	});
	^dataArr;
}
